...
function renderDate() {
	...
	for (let i = 0; i < daysInMonth; i++) {
		dateEle.innerHTML += `
            <button onclick = 'handleRedirect(`+ (i + 1) + `)' class="day ${activeCurrentDay(i + 1)}">${i + 1}</button>
        `;
	}
}


//Lấy ra ngày, tháng, năm và redirect
function handleRedirect(C_day) {
	document.getElementById("i_day").value = C_day;
	document.getElementById("i_month").value = currentMonth + 1;
	document.getElementById("i_year").value = currentYear;
	document.getElementById("form_task").submit();

	//alert(C_day + " "+ currentMonth + " "  + currentYear);
}
